...
const DICTIONARIES = {
  ...
  fr: {
    ...
    result: { tier: { goleada: 'ROUSTE !', amplia: 'Large victoire', ajustada: 'Victoire serrée', empate: 'Match nul', derrota: 'Défaite' }, lostStep: 'Échelon perdu', towerFall: 'Chute de la tour', retry: 'Retenter l’échelon', nextLevel: 'Niveau suivant', reward: 'Récompense', lossCopy: ({ lives }) => `Vous avez perdu cet échelon. Il vous reste ${lives} vie${lives === 1 ? '' : 's'}.`, rewardCopy: ({ players, items }) => `Récompense prête : pack de ${players} joueurs et ${items} objets à choisir.`, scorers: 'Buteurs', reds: 'Expulsés', saves: 'Arrêts', lastMatch: 'Dernier match', gameOver: 'Fin de run', floorsReached: 'étages atteints', newRecord: '★ Nouveau record', best: ({ best }) => `Meilleure marque : ${best}`, winsRoster: ({ wins, count }) => `${wins} victoire${wins === 1 ? '' : 's'} · effectif de ${count} joueurs`, route: 'Parcours', finalSquad: 'Effectif final', playAgain: 'Rejouer', pathLevel: ({ level }) => `Nv ${level}` },
